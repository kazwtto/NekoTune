...
function MoodGenreCard({ tag, index }: { tag: MoodGenre; index: number }) {
  ...
  return (
    ...
    >
      <div className="flex h-[40px] w-full items-end p-2 overflow-hidden">
        <p className="w-full truncate text-[11px] font-bold leading-tight text-white drop-shadow-md">{tag.title}</p>
      </div>
    </div>
  )
}
