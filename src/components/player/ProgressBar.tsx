...
export default function ProgressBar({ showTime = true, color = "white", onSeek }: { showTime?: boolean; color?: string, onSeek?: (time: number) => void }) {
  ...
  return (
    <div className="flex w-full items-center gap-3">
      ...
      <div className="group relative h-1 flex-1 cursor-pointer rounded-full bg-white/10 flex items-center">
        <div
          className={`absolute left-0 h-full rounded-full bg-[${color}] transition-all pointer-events-none`}
          style={{ width: `${pct}%`, transitionDuration: isDragging ? '0ms' : '100ms' }}
        />
        <div
          className={`absolute h-3.5 w-3.5 -translate-x-1/2 rounded-full bg-[${color}] shadow-md transition-opacity pointer-events-none ${
